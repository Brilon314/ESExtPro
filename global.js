...
/*var getGob = imperioFantasma.cargaFantasma("https://www.empire-strike.com/gobierno.php", imperioFantasma.getGobiernantes, LOCAL.setGobernantes);
console.log('getGob:', getGob);

var prueba1= LOCAL.getImperio();
console.log('prueba1:', prueba1);
var prueba2= imperioFantasma.getPoliticas();
...
var imperioFantasma = {
  cargaImperio: function () {
    ...
    if (LOCAL.getClan() == null) {
      if ((LOCAL.getImperio() && LOCAL.getImperio().clan != "") || (imperioFantasma.getMaravillas != null)){
        console.log("Detectamos el id?");
        console.log("LOCAL.getImperio().clan:", LOCAL.getImperio().clan);
        var urlClan = "https://www.empire-strike.com/clan.php?id=" + LOCAL.getImperio().clan;
        imperioFantasma.cargaFantasma(urlClan, imperioFantasma.getMaravillas, LOCAL.setClan);
      }
    }
    imperioFantasma.recogerSiglasYLinks();
    console.log('imperioFantasma.recogerSiglasYLinks():', imperioFantasma.recogerSiglasYLinks());
    // if (LOCAL.getClan() == null && LOCAL.getImperio().clan != "") {
    /*if (LOCAL.getClan() == null || LOCAL.getImperio().clan != "") {
    	console.log("Detectamos el id?");
    	// var urlClan = "https://www.empire-strike.com/clanes.php";
    	console.log('LOCAL.getImperio().clan:', LOCAL.getImperio().clan);
    	var urlClan = location.origin+"/clan.php?sclan="+LOCAL.getImperio().clan;
    	imperioFantasma.cargaFantasma(urlClan, imperioFantasma.getMaravillas, LOCAL.setClan);
    }*/
  },
  recogerSiglasYLinks: function () {
    console.log("Entramos recogerSiglasYLinks:");
    var filas = document.querySelectorAll("#contenido > table > tbody > tr");
    console.log("filas:", filas);
    // Empezamos desde el segundo elemento (índice 1) ya que mencionaste que es el primero que quieres.
    for (var i = 1; i < filas.length; i++) {
      var celda = filas[i].querySelector("td:nth-child(2)");
      console.log("celda:", celda);
      // Obteniendo las siglas
      var texto = celda.textContent;
      console.log("texto:", texto);
      var coincidencia = texto.match(/\(([^)]+)\)/); // Busca texto entre paréntesis
      if (!coincidencia) continue; // Si no encuentra, pasa al siguiente elemento
      console.log("coincidencia:", coincidencia);
      var siglas = coincidencia[1];
      console.log("siglas:", siglas);
      // Obteniendo el enlace
      var enlaceElemento = celda.querySelector("a");
      console.log("enlaceElemento:", enlaceElemento);
      var enlace = enlaceElemento ? enlaceElemento.href : null;
    ...
    // Ejecutar la función
    var resultado = imperioFantasma.recogerSiglasYLinks();
    console.log(resultado);
    var resultado2 = recogerSiglasYLinks();
    console.log('resultado2:', resultado2);
    var clanActual = LOCAL.getImperio().clan;
    console.log("LOCAL.getImperio().clan:", LOCAL.getImperio().clan);
    console.log("clanActual:", clanActual);
    var enlaceDelClan = null; // Aquí guardaremos el enlace si encontramos una coincidencia.
    //	Obtención de enlaces para clanes
    for (var asd = 0; asd < resultado.length; asd++) {
      if (resultado[asd].siglas === clanActual) {
        enlaceDelClan = resultado[i].enlace;
        console.log("enlaceDelClan:", enlaceDelClan);
        break; // Salimos del bucle una vez que encontramos una coincidencia.
      }
    }
    if (enlaceDelClan) {
      console.log("¡Clan encontrado!");
      console.log("Enlace al clan:", enlaceDelClan);
    } else {
      console.log("Clan no encontrado en la lista.");
    }
    return enlaceDelClan;
  },
  ...
  getMaravillas: function (doc) {
    ...
    /* if (document.getElementById("_ayudam1") == null) return miClan;
     console.log('document.querySelector("#_ayudam1 h3").innerText:', document.querySelector("#_ayudam1 h3").innerText);
     miClan.maravilla1 = document.querySelector("#_ayudam1 h3").innerText;
     if (document.getElementById("_ayudam2") == null) return miClan;
     console.log('document.querySelector("#_ayudam2 h3").innerText:', document.querySelector("#_ayudam2 h3").innerText);
  ...
  getGobiernantes: function (doc) {
    ...
    console.log("gobernantes:", gobernantes);
    return gobernantes;
  },
